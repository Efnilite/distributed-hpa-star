...
static inline Vec2 global_vec_to_local_vec(const MasterCluster* cluster, Vec2 vec)
{
    return (Vec2){vec.x - cluster->pos.x * CLUSTER_SIZE, vec.y - cluster->pos.y * CLUSTER_SIZE};
}

static inline Vec2 local_vec_to_global_vec(const MasterCluster* cluster, Vec2 vec)
{
    return (Vec2){vec.x + cluster->pos.x * CLUSTER_SIZE, vec.y + cluster->pos.y * CLUSTER_SIZE};
}
