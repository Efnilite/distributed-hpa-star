...
int tcp_client_is_connected(tcp_client* client) { return client && client->connected; }

int tcp_client_get_socket_fd(tcp_client* client) { return (client && client->connected) ? client->socket_fd : -1; }
...
{
    if (socket_fd < 0 || !header || !out_payload)
        return -1;
...
int tcp_send_task_response(int socket_fd, const TaskResponse* response)
{
    ...
    uint32_t response_header_size =
        sizeof(uint32_t) * 2 + sizeof(int32_t) + sizeof(uint16_t) + sizeof(long) + sizeof(double); // task_id, path_length, status_code, max memory, time, worker_id
